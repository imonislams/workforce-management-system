...
@section('header', 'Create Designation')

@section('content')
<div class="max-w-2xl mx-auto bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm">
    <h2 class="text-xl font-bold text-slate-900 mb-6">Create New Designation</h2>

    <form action="{{ route('admin.designations.store') }}" method="POST" class="space-y-5">
        @csrf
        <div>
        ...
        <div class="flex items-center justify-end space-x-3 pt-4">
            ...
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-500 shadow">Save Designation</button>
        </div>
    </form>
</div>
@endsection

...
<div class="max-w-2xl mx-auto bg-white rounded-2xl border border-slate-200/80 p-8 shadow-sm">
    <h2 class="text-xl font-bold text-slate-900 mb-6">Create System User</h2>
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="password" name="password" required class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm focus:outline-none focus:border-indigo-500">
        </div>
        ...
        <div class="flex items-center justify-end space-x-3 pt-4">
            ...
            <button type="submit" class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-500 shadow">Save User</button>
        </div>
    </form>
</div>
@endsection
